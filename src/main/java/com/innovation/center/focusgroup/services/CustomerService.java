...
public class CustomerService {
    ...
    public List<Customer> searchUsersByName(String username) {
        if (username == null || username.trim().isEmpty()) {
            // If the username is not provided, return all customers
            return customerRepository.findAll();
        }

        // If the username is provided, perform the search
        List<Customer> customers = customerRepository.findByUsernameContainingIgnoreCase(username);

        if (customers.isEmpty()) {
            // Throw an exception when no customers with the specified username were found
            throw new ApplicationException.UserNotFoundException("No customers found with the specified username: " + username);
        }
